...
public class Assorted {
    ...
    public static int findSum(List<?> list) {
        ...
        list2 = (ArrayList<Integer>) list; int count = 0;
        for(int i = 0; i < list2.size();){
            count = count + toInteger(list2.get(i));
            i++;
        }
        return count;
    }
     ...
     */
    public static int busStop(List<Integer[]> list) { ///bruh
        int peopleleft = 0;
        for(int i = 0; i < list.size(); i++){
            Integer[] what = list.get(i);
            peopleleft = peopleleft + what[i];
